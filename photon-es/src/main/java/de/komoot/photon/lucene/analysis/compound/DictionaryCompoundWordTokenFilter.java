...
public class DictionaryCompoundWordTokenFilter extends CompoundWordTokenFilterBase {
  ...
  protected void decompose() {
    ...
    // retain original compound word, first has posInc=1, no support for posLength
    tokens.add(new CompoundToken(0,len,1));
                ...
                if (this.onlyLongestMatch) {
                   if (longestMatchToken!=null) {
                     if (longestMatchToken.txt.length()<j) {
                       longestMatchToken=new CompoundToken(i,j, 0);
                     }
                   } else {
                     longestMatchToken=new CompoundToken(i,j, 0);
                   }
                } else {
                   tokens.add(new CompoundToken(i,j, 0));
                }
            } 
        }
