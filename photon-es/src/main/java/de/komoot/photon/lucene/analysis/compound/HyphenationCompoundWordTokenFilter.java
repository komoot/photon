...
public class HyphenationCompoundWordTokenFilter extends CompoundWordTokenFilterBase {
    ...
    private int pendingPosIncrement;
     ...
     */
    public HyphenationCompoundWordTokenFilter(TokenStream input, HyphenationTree hyphenator,
                    CharArraySet dictionary, int minWordSize, int minSubwordSize,
     ...
     * <p>
     * Calls
     * {@link #HyphenationCompoundWordTokenFilter(org.apache.lucene.analysis.TokenStream, org.apache.lucene.analysis.compound.hyphenation.HyphenationTree, org.apache.lucene.analysis.CharArraySet, int, int, int, boolean)
     * HyphenationCompoundWordTokenFilter(matchVersion, input, hyphenator, null,
     ...
     * <p>
     * Calls
     * {@link #HyphenationCompoundWordTokenFilter(org.apache.lucene.analysis.TokenStream, org.apache.lucene.analysis.compound.hyphenation.HyphenationTree, int, int, int)
    ...
    protected void decompose() {
        ...
        for (int i = 0; i < hyp.length; ++i) {
            ...
            for (int j = remaining - 1; j > 0; j--) {
                ...
                if (i == 0 && j == remaining - 1) {
                    // explicitly add compound word, but with updated
                    // positionLength,
                    // as it is returned first of all subwords, it's posInc is 1
                    tokens.add(new CompoundToken(start, partLength, pendingPosIncrement, j));
                    pendingPosIncrement = 0;
                    // the compound word is no candidate for the longest match
                    continue;
                }
                // we only put subwords to the token stream
                // that don't exceed maxSubwordSize
                if (partLength > this.maxSubwordSize) {
                    continue;
                }

                // if the part is shorter than minSubwordSize we
                // are done with this round
                if (partLength < this.minSubwordSize) {
                    // BOGUS/BROKEN/FUNKY/WACKO: somehow we have negative
                    // 'parts' according to the
                    // calculation above, and we rely upon minSubwordSize being
                    // >=0 to filter them out...
                    break;
                }
                ...
                if (dictionary == null
                    ...
                    // check the dictionary again with a word that is one
                    // character
                    // shorter
                    // to avoid problems with genitive 's characters and other
                    // binding
                    // characters
                    if (this.onlyLongestMatch) {
                        if (longestMatchToken != null) {
                        ...
                        tokens.add(new CompoundToken(start, partLength - 1, pendingPosIncrement,
                                        j));
                        pendingPosIncrement = 0;
                    }
                }
    ...
    private void updatePosIncrement() {
        this.posIncAtt.setPositionIncrement(
                        this.posIncAtt.getPositionIncrement() + pendingPosIncrement);
        pendingPosIncrement = 0;
    }
